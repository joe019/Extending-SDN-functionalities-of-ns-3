...
OpenFlowSwitchNetDevice::AddVPort (const ofp_vport_mod *ovpm)
{
	...
	NS_LOG_INFO("\n\n===Before virtual --- : "<<ntohl(ovpm->vport));
          ...
          if (packet->PeekHeader (tcp_hd))
            {
              ...
              NS_LOG_INFO ("---kkkkkkkkkkkkkkkkkkkkkk---tcp dst is " << tcp_h->tcp_src );
              tcp_h->tcp_dst = htons (tcp_hd.GetDestinationPort ());    // Destination Port
              NS_LOG_INFO ("---kkkkkkkkkkkkkkkkkkkkkk---tcp dst is " << tcp_h->tcp_dst );
...
OpenFlowSwitchNetDevice::OutputControl (uint32_t packet_uid, int in_port, size_t max_len, int reason)
{
  ...
  ofi::SwitchPacketMetadata dataagm = m_packetData.find (packet_uid)->second;
  ofpbuf* buffer = dataagm.buffer;
  ...
  opi->reason         = reason;
  opi->pad            = 0;
  Packet packetxyz = Packet((uint8_t*)(buffer->data),buffer->size);
      NS_LOG_INFO (packetxyz.GetUid());
...
OpenFlowSwitchNetDevice::FlowTableLookup (sw_flow_key key, ofpbuf* buffer, uint32_t packet_uid, int port, bool send_to_controller)
{
 ...
 /* if (flow != 0)
    ...
    {*/
      NS_LOG_INFO ("modified @agm@  Flow not matched.");
